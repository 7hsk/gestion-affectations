...
<head>
    ...
    <title>Emploi du Temps - {{ $filiere->nom }} {{ $semester }}</title>
    <style>
        @page {
            margin: 20mm;
            size: A4 landscape;
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-center">
            ...
            <div class="filiere-info">{{ $filiere->nom }}</div>
            <div class="semester-info">{{ $semester }}</div>
        ...
        <div class="header-right">
            <div class="export-info">
                ...
                Coordonnateur: {{ $coordonnateur->name }}<br>
                <small style="margin-top: 5px; display: block;">
                    {{ $exportInfo['total_courses'] ?? 0 }} cours programmés<br>
                    {{ $exportInfo['unique_ues'] ?? 0 }} UEs différentes<br>
                    {{ $exportInfo['assigned_teachers'] ?? 0 }} enseignants assignés
                </small>
            </div>
        </div>
    </div>
                                ...
                                @foreach($groupedSchedules[$day][$timeSlot] as $schedule)
                                    <div class="schedule-item {{ strtolower($schedule->type_seance) }}">
                                        ...
                                        <span class="session-type">
                                            ...
                                            @if($schedule->groupe_numero && in_array($schedule->type_seance, ['TD', 'TP']))
                                                -G{{ $schedule->groupe_numero }}
                                            @endif
                                        </span>
                                        <span class="teacher-name">{{ $schedule->user->name ?? 'Non assigné' }}</span>
                                    </div>
                                @endforeach
                            @endif
                        </td>
    ...
    @if($schedules->isEmpty())
        <div style="text-align: center; margin: 30px 0; padding: 20px; background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 5px;">
            <p style="color: #6c757d; font-style: italic; margin: 0;">
                Aucun cours programmé pour {{ $filiere->nom }} - {{ $semester }}.
            </p>
            <small style="color: #888; margin-top: 10px; display: block;">
                État actuel de l'interface de gestion des emplois du temps
            </small>
        </div>
    @else
        <div style="text-align: center; margin: 20px 0; padding: 10px; background-color: #e8f5e8; border: 1px solid #059669; border-radius: 5px;">
            <small style="color: #059669; font-weight: 500;">
                📅 État actuel sauvegardé de l'emploi du temps ({{ $schedules->count() }} créneaux programmés)
            </small>
        </div>
    @endif
    ...
    <div class="footer">
        ...
        <p>ENSA Al Hoceima - École Nationale des Sciences Appliquées - {{ $filiere->nom }} ({{ $semester }})</p>
    </div>
</body>
</html>
