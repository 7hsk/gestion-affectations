...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('coordonnateur.unites-enseignement') }}" class="btn btn-cancel">
    ...
    <div class="form-container">
        <form action="{{ route('coordonnateur.unites-enseignement.update', $ue->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <!-- Filiere and Semester -->
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="filiere_id" class="form-label">Filière *</label>
                        <select class="form-select @error('filiere_id') is-invalid @enderror" 
                                id="filiere_id" name="filiere_id" required onchange="updateSemesterOptions()">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        @error('semestre')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                        <div class="semester-info" id="semesterInfo" style="display: none;">
                            <i class="fas fa-info-circle me-1"></i>
                            <span id="semesterText"></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <div class="specialite-selector">
                            <small class="text-muted">Sélectionnez les spécialités requises pour cette UE</small>
                            <div class="specialite-grid">
                                @foreach($allSpecialites as $spec)
                                    @php
                                        $currentSpecialites = old('specialite', $ue->specialite ? explode(',', $ue->specialite) : []);
                                        $isSelected = in_array(trim($spec), array_map('trim', $currentSpecialites));
                                    @endphp
                                    <div class="specialite-item {{ $isSelected ? 'selected' : '' }}">
                                        <input type="checkbox" name="specialite[]" value="{{ $spec }}" 
                                               id="spec_{{ $loop->index }}" {{ $isSelected ? 'checked' : '' }}>
                                        <label for="spec_{{ $loop->index }}">{{ $spec }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update semester options based on filiere
...
function updateSemesterOptions() {
    ...
    // Determine available semesters based on filiere name
    let availableSemesters = [];
    let infoText = '';
    
    if (filiereName.endsWith('1')) {
        availableSemesters = ['S1', 'S2'];
        infoText = 'Première année: S1 et S2 disponibles';
    } else if (filiereName.endsWith('2')) {
        availableSemesters = ['S3', 'S4'];
        infoText = 'Deuxième année: S3 et S4 disponibles';
    } else if (filiereName.endsWith('3')) {
        availableSemesters = ['S5'];
        infoText = 'Troisième année: S5 disponible';
    } else {
        availableSemesters = ['S1', 'S2', 'S3', 'S4', 'S5'];
        infoText = 'Tous les semestres disponibles (S1 à S5)';
    }
    ...
    // Show info
    semesterText.textContent = infoText;
